        ...
        if (empty($username) || empty($first_name) || empty($last_name) || empty($email) || empty($password)) {
            ...
            try {
                ...
                if ($stmt->fetch()) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO users (username, password, role, first_name, last_name, email, status) VALUES (?, ?, 'client', ?, ?, ?, ?)");
                    $stmt->execute([$username, $hashedPassword, $first_name, $last_name, $email, $status]);
                    $userId = $conn->lastInsertId();
                    
                    // Create client profile
                    $stmt = $conn->prepare("INSERT INTO client_profiles (user_id, company_name, total_spent, jobs_posted, jobs_completed) VALUES (?, ?, 0.00, 0, 0)");
                    $stmt->execute([$userId, $company_name]);
                    ...
                    logAdminAdd($_SESSION['user_id'], 'client', $newValues, 'Admin added new client');
        ...
        if (!$id || empty($username) || empty($first_name) || empty($last_name) || empty($email)) {
            ...
            try {
                ...
                if ($stmt->fetch()) {
                    ...
                    $oldStmt = $conn->prepare("SELECT u.username, u.first_name, u.last_name, u.email, u.status, cp.company_name FROM users u LEFT JOIN client_profiles cp ON u.id = cp.user_id WHERE u.id = ?");
                    $oldStmt->execute([$id]);
                    $oldValues = $oldStmt->fetch();
                    
                    // Update user
                    $stmt = $conn->prepare("UPDATE users SET username = ?, first_name = ?, last_name = ?, email = ?, status = ? WHERE id = ? AND role = 'client'");
                    $stmt->execute([$username, $first_name, $last_name, $email, $status, $id]);
                    
                    // Update client profile
                    $stmt = $conn->prepare("UPDATE client_profiles SET company_name = ? WHERE user_id = ?");
                    ...
                    logAdminEdit($_SESSION['user_id'], 'client', $id, $oldValues, $newValues, 'Admin updated client information');
    ...
    if ($id) {
        try {
            ...
            if ($client) {
                ...
                logAdminEdit($_SESSION['user_id'], 'client', $id, $oldValues, $newValues, 'Admin changed client status');
...
if (isset($_GET['delete'])) {
    ...
    if ($id) {
        try {
            // Get client info
            $stmt = $conn->prepare("SELECT username FROM users WHERE id = ? AND role = 'client'");
            $stmt->execute([$id]);
            ...
            if ($clientToDelete) {
                ...
                logAdminDelete($_SESSION['user_id'], 'client', $id, $oldValues, 'Admin deleted client');
                
                $success = "Client '" . htmlspecialchars($clientToDelete['username']) . "' deleted successfully.";
            } else {
                $error = "Client not found.";
            }
        } catch (PDOException $e) {
            error_log("Delete client error: " . $e->getMessage());
            $error = "Error deleting client. The client may have associated data.";
        }
    }
}
...
try {
    ...
    $query = "SELECT u.id, u.username, u.first_name, u.last_name, u.email, u.status, u.created_at,
              ...
              WHERE u.role = 'client'";
    $countQuery = "SELECT COUNT(*) FROM users u WHERE u.role = 'client'";
            ...
            <div class="flex items-center justify-between flex-wrap gap-4">
                ...
                <div class="flex items-center gap-3">
                    <span class="px-4 py-2 bg-white rounded-lg shadow-md border border-gray-200 text-sm font-semibold text-gray-700">
                        <i class="ri-briefcase-line text-green-600"></i> Total: <span class="text-green-600"><?php echo $totalClients; ?></span>
                    </span>
                </div>
            </div>
        </div>
                ...
                <div class="p-6">
                    <form method="GET" class="space-y-4">
                            ...
                            <div class="group">
                                ...
                                <div class="relative">
                                    <input 
                                        ...
                                        id="search" 
                                        name="search" 
                                        class="w-full pl-11 pr-4 py-3 border-2 border-gray-300 rounded-xl transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-green-100 focus:border-green-500 hover:border-green-400" 
                            ...
                            <div class="group">
                                ...
                                <div class="relative">
                                    <select 
                                        id="status" 
                                        name="status" 
                                        class="w-full pl-11 pr-4 py-3 border-2 border-gray-300 rounded-xl transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-green-100 focus:border-green-500 hover:border-green-400 appearance-none bg-white cursor-pointer">
                        ...
                        <div class="flex gap-3 pt-2">
                            <button 
                                type="submit" 
                                class="btn-ripple flex-1 sm:flex-none px-6 py-3 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white rounded-xl transition-all duration-300 flex items-center justify-center gap-2 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 font-semibold">
                ...
                <div class="flex flex-wrap justify-between items-center gap-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600"><?php echo $totalClients; ?> total clients found</p>
                        </div>
                    </div>
                    <button 
                        id="addClientBtn" 
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider">Company</th>
                            <th class="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider">Total Spent</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h4 class="text-xl font-bold text-gray-700 mb-2">No Clients Found</h4>
                                    <p class="text-gray-500 text-sm">Start by adding your first client</p>
                                </div>
                            </td>
                        </tr>
                        <?php else: ?>
                        ...
                        <tr class="table-row-hover">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="text-sm font-bold text-gray-900">#<?php echo htmlspecialchars($client['id']); ?></span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="text-sm text-gray-900">
                                    <span class="font-semibold"><?php echo htmlspecialchars($client['jobs_posted'] ?? 0); ?></span> / 
                                    <span class="text-green-600 font-semibold"><?php echo htmlspecialchars($client['jobs_completed'] ?? 0); ?></span>
                                </div>
                                <div class="text-xs text-gray-500">Posted / Completed</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="<?php 
                                    echo $client['status'] === 'active' 
                                        ? 'bg-green-100 text-green-800 border border-green-200' 
                                        : 'bg-red-100 text-red-800 border border-red-200';
                                ?> px-3 py-1.5 rounded-full text-xs font-bold inline-flex items-center gap-1.5 capitalize shadow-sm">
                                    <?php echo $client['status'] === 'active' ? '✅ Active' : '❌ Inactive'; ?>
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-2 flex-wrap">
                                    <button 
                                        onclick="editClient(<?php echo $client['id']; ?>, '<?php echo htmlspecialchars($client['username'], ENT_QUOTES); ?>', '<?php echo htmlspecialchars($client['first_name'], ENT_QUOTES); ?>', '<?php echo htmlspecialchars($client['last_name'], ENT_QUOTES); ?>', '<?php echo htmlspecialchars($client['email'], ENT_QUOTES); ?>', '<?php echo htmlspecialchars($client['company_name'] ?? '', ENT_QUOTES); ?>', '<?php echo htmlspecialchars($client['status']); ?>')"
                                    ...
                                    <button 
                                        onclick="confirmToggleStatus(<?php echo $client['id']; ?>, '<?php echo $client['status']; ?>', '<?php echo htmlspecialchars($client['first_name'] . ' ' . $client['last_name'], ENT_QUOTES); ?>')"
                                    ...
                                    <button 
                                        onclick="confirmDelete(<?php echo $client['id']; ?>, '<?php echo htmlspecialchars($client['username'], ENT_QUOTES); ?>')"
                    ...
                    <div class="bg-gradient-to-r from-green-50 to-emerald-50 p-4 border-b border-gray-200">
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex items-center gap-3 flex-1">
                                <div class="h-12 w-12 rounded-full bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center text-white font-bold shadow-lg flex-shrink-0">
                                    <?php echo strtoupper(substr($client['first_name'], 0, 1) . substr($client['last_name'], 0, 1)); ?>
                                </div>
                                <div class="min-w-0">
                                    ...
                                    <p class="text-sm text-gray-600 flex items-center gap-1">
                                        ...
                                        <span class="truncate"><?php echo htmlspecialchars($client['username']); ?></span>
                                    </p>
                                </div>
                            </div>
                            <span class="<?php 
                    ...
                    <div class="p-4 space-y-3">
                        <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-sm text-gray-900 font-medium truncate"><?php echo htmlspecialchars($client['email']); ?></p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                            ...
                            <div class="flex-1">
                                <p class="text-xs text-gray-500 font-semibold uppercase mb-0.5">Company</p>
                                <p class="text-sm text-gray-900 font-medium"><?php echo $client['company_name'] ? htmlspecialchars($client['company_name']) : '-'; ?></p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                                ...
                                <div class="flex-1">
                                    <p class="text-xs text-gray-500 font-semibold uppercase mb-0.5">Spent</p>
                                    <p class="text-sm text-green-600 font-bold">$<?php echo $client['total_spent'] ? number_format($client['total_spent'], 2) : '0.00'; ?></p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-sm text-gray-900 font-bold"><?php echo htmlspecialchars($client['jobs_posted'] ?? 0); ?> / <?php echo htmlspecialchars($client['jobs_completed'] ?? 0); ?></p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                            ...
                            <div class="flex-1">
                                <p class="text-xs text-gray-500 font-semibold uppercase mb-0.5">Client ID</p>
                                <p class="text-sm text-gray-900 font-bold">#<?php echo htmlspecialchars($client['id']); ?></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2 flex-wrap justify-center">
                        <?php if ($page > 1): ?>
                            <a 
                                ...
                                class="px-4 py-2 bg-white border-2 border-gray-300 text-gray-700 rounded-lg hover:bg-green-600 hover:text-white hover:border-green-600 transition-all duration-200 font-semibold text-sm shadow-sm hover:shadow-md">
                        ...
                        if ($start > 1): ?>
                            <a href="?<?php echo http_build_query(array_merge($_GET, ['page' => 1])); ?>" class="hidden sm:block px-4 py-2 bg-white border-2 border-gray-300 text-gray-700 rounded-lg hover:bg-green-600 hover:text-white hover:border-green-600 transition-all duration-200 font-semibold text-sm shadow-sm hover:shadow-md">1</a>
                            <?php if ($start > 2): ?>
                        ...
                        <?php for ($i = $start; $i <= $end; $i++): ?>
                            <?php if ($i == $page): ?>
                                <span class="px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-600 border-2 border-green-600 text-white rounded-lg font-bold text-sm shadow-md"><?php echo $i; ?></span>
                            <?php else: ?>
                                <a href="?<?php echo http_build_query(array_merge($_GET, ['page' => $i])); ?>" class="hidden sm:block px-4 py-2 bg-white border-2 border-gray-300 text-gray-700 rounded-lg hover:bg-green-600 hover:text-white hover:border-green-600 transition-all duration-200 font-semibold text-sm shadow-sm hover:shadow-md"><?php echo $i; ?></a>
                            <?php endif; ?>
                        <?php endfor; ?>
...
<!-- Add Client Modal -->
<div id="addClientModal" class="hidden fixed inset-0 z-50 overflow-y-auto modal-backdrop" style="background-color: rgba(0, 0, 0, 0.75);">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0 mt-12">
        <div class="fixed inset-0 transition-opacity" onclick="closeModal('addClientModal')"></div>
        ...
        <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full animate-scale-in">
            <div class="bg-gradient-to-r from-green-600 to-emerald-600 px-6 py-5">
                <div class="flex items-center justify-between">
                    ...
                    <button onclick="closeModal('addClientModal')" class="text-white hover:text-gray-200 transition-colors">
            ...
            <form method="POST" class="p-6 space-y-5">
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                        <i class="ri-user-line text-green-600"></i> Username
                    </label>
                    <input 
                        type="text" 
                        name="username" 
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                            <i class="ri-user-line text-green-600"></i> First Name
                        </label>
                        <input 
                            type="text" 
                            name="first_name" 
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                            <i class="ri-user-line text-green-600"></i> Last Name
                        </label>
                        <input 
                            type="text" 
                            name="last_name" 
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                        <i class="ri-mail-line text-green-600"></i> Email
                    </label>
                    <input 
                        type="email" 
                        name="email" 
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                        <i class="ri-building-line text-green-600"></i> Company Name
                    </label>
                    <input 
                        type="text" 
                        name="company_name" 
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                        <i class="ri-lock-password-line text-green-600"></i> Password
                    </label>
                    <input 
                        type="password" 
                        name="password" 
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                        <i class="ri-toggle-line text-green-600"></i> Status
                    </label>
                    <select 
                        name="status" 
                        required 
...
<!-- Edit Client Modal -->
<div id="editClientModal" class="hidden fixed inset-0 z-50 overflow-y-auto modal-backdrop" style="background-color: rgba(0, 0, 0, 0.75);">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0 mt-12">
        <div class="fixed inset-0 transition-opacity" onclick="closeModal('editClientModal')"></div>
        ...
        <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full animate-scale-in">
            <div class="bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-5">
                <div class="flex items-center justify-between">
                    ...
                    <button onclick="closeModal('editClientModal')" class="text-white hover:text-gray-200 transition-colors">
                        <i class="ri-close-line text-2xl"></i>
                    </button>
                </div>
            </div>
            ...
            <form method="POST" class="p-6 space-y-5">
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                        <i class="ri-building-line text-indigo-600"></i> Company Name
                    </label>
                    <input 
                        type="text" 
        ...
        <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full animate-scale-in">
            <div class="bg-gradient-to-r from-blue-600 to-cyan-600 px-6 py-5">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-xl font-bold text-white">Toggle Client Status</h3>
                </div>
            </div>
        ...
        <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full animate-scale-in">
            <div class="bg-gradient-to-r from-red-600 to-rose-600 px-6 py-5">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-xl font-bold text-white">Delete Client</h3>
                </div>
            </div>
                    ...
                    <div class="bg-red-50 border-l-4 border-red-500 p-4 rounded-lg">
                        <div class="flex items-start">
                            ...
                            <div class="text-left">
                                ...
                                <p class="text-xs text-red-700 mt-1">All client data will be permanently deleted from the system.</p>
                            </div>
                        </div>
                    </div>
                </div>
...
function confirmToggleStatus(clientId, currentStatus, clientName) {
    const action = currentStatus === 'active' ? 'deactivate' : 'activate';
    const actionText = currentStatus === 'active' ? 'Deactivate' : 'Activate';
    
    document.getElementById('toggleModalTitle').textContent = `${actionText} ${clientName}?`;
    document.getElementById('toggleModalMessage').textContent = `Are you sure you want to ${action} this client account?`;
    document.getElementById('confirmToggleBtn').href = `?toggle_status=${clientId}`;
    openModal('confirmToggleModal');
}

// Confirm Delete
function confirmDelete(clientId, username) {
    document.getElementById('deleteModalMessage').textContent = `You are about to delete the client "${username}".`;
    document.getElementById('confirmDeleteBtn').href = `?delete=${clientId}`;
...
function editClient(id, username, firstName, lastName, email, companyName, status) {
